...
static const char WEB_INDEX_HTML[] =
    ...
    "g=v?221:0,i=(y*w+px)*4;img.data[i]=img.data[i+1]=img.data[i+2]=g;"
    "img.data[i+3]=255}}return img}"
    "function pix2(data,w,h){const img=x.createImageData(w,h),p=[0,85,170,255];"
    "for(let y=0;y<h;y++){let ro=(y&1?0x2000:0)+((y>>1)*80);"
    "for(let px=0;px<w;px++){let b=data[ro+(px>>2)],v=(b>>(6-((px&3)*2)))&3,"
    "g=p[v],i=(y*w+px)*4;img.data[i]=img.data[i+1]=img.data[i+2]=g;"
    "img.data[i+3]=255}}return img}"
    ...
    "x.putImageData(bpp===1?pix1(data,w,h):pix2(data,w,h),0,0);"
