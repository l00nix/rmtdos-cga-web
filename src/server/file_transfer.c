...
#define FILE_OP_BEGIN 1
#define FILE_OP_DATA 2
#define FILE_OP_END 3
...
static void send_file_ack(uint16_t command, uint16_t status) {
  ...
  ack->offset = htonl(g_file.expected_offset);
...
void file_transfer_handle_packet(const struct Buffer *buffer) {
  ...
  switch (pkt_type) {
    case V1_FILE_PUT_BEGIN: {
      ...
      g_file.pending_op = FILE_OP_BEGIN;
    } break;
    ...
    case V1_FILE_PUT_DATA: {
      ...
      if (!g_file.active || transfer_id != g_file.transfer_id) {
        return;
      }
      ...
      g_file.pending_op = FILE_OP_DATA;
    } break;
